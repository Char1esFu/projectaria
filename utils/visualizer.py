...
class AriaVisualizerStreamingClientObserver(BaseStreamingClientObserver):
    ...
    def on_image_received(self, image: np.array, record: ImageDataRecord) -> None:
        ...
        # Calculate and print FPS and resolution
        current_time = time.time()
        camera_id = record.camera_id
        
        if camera_id not in self.last_print_time:
            self.last_print_time[camera_id] = current_time
            self.frame_counts[camera_id] = 0
            
        self.frame_counts[camera_id] += 1
        
        if current_time - self.last_print_time[camera_id] >= 1.0:
            elapsed = current_time - self.last_print_time[camera_id]
            fps = self.frame_counts[camera_id] / elapsed
            print(f"Camera {camera_id}: {fps:.2f} FPS, Resolution: {image.shape}")
            self.last_print_time[camera_id] = current_time
            self.frame_counts[camera_id] = 0
    ...
    def on_imu_received(self, samples: Sequence[MotionData], imu_idx: int) -> None:
        ...
        if key not in self.last_print_time:
            self.last_print_time[key] = current_time
            self.frame_counts[key] = 0
            
        self.frame_counts[key] += len(samples)
        
        if current_time - self.last_print_time[key] >= 1.0:
            elapsed = current_time - self.last_print_time[key]
            freq = self.frame_counts[key] / elapsed
            print(f"IMU {imu_idx}: {freq:.2f} Hz")
            self.last_print_time[key] = current_time
            self.frame_counts[key] = 0
    ...
    def on_magneto_received(self, sample: MotionData) -> None:
        ...
        if current_time - self.last_print_time[key] >= 1.0:
            elapsed = current_time - self.last_print_time[key]
            freq = self.frame_counts[key] / elapsed
            print(f"Magnetometer: {freq:.2f} Hz")
            self.last_print_time[key] = current_time
            self.frame_counts[key] = 0
