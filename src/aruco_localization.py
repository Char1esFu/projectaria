...
class ArucoLocalizer:
        ...
        while not quit_keypress():
            ...
            if ids is not None and len(ids) > 0:
                ...
                for marker_id, corner_set, rvec, tvec in zip(
                    ...
                    static_entry = self.static_marker_transforms.get(int(marker_id))
                    if static_entry is None:
                        if int(marker_id) not in self._warned_missing_static:
                            print(
                                f"No static TF for marker {int(marker_id)}; cannot publish world cam pose."
                            )
                            self._warned_missing_static.add(int(marker_id))
                        continue
                ...
                if allowed_id_set is None:
                    with_poses += len(ids)
                else:
                    with_poses += sum(int(marker_id) in allowed_id_set for marker_id in ids.flatten())
    ...
    def _setup_ros(self) -> None:
        try:
            ...
            self.ros_publisher = self.ros_node.create_publisher(PoseStamped, self.ros2_topic, 10)
            ...
            print(f"ROS2 publishing enabled: {self.ros2_topic}")
        except Exception as exc:
    ...
    def _publish_static_tf(self) -> None:
        ...
        try:
            ...
            for entry in config_data.get("static_transforms", []):
                ...
                child_frame = f"{self.ros2_marker_frame_prefix}{marker_id}"
